"""
models/evidence/model.py — Evidence domain model.

Raw evidence collected by the Argus agent for a single verification run.
Stores both the summarised stats (commit counts, confidence) and
the full raw JSON payloads for audit purposes.
"""
...
class Evidence(TimestampMixin, Base):
    """Argus agent output for one milestone verification run."""
    ...
    pull_requests_merged: Mapped[int] = mapped_column(
    ...
    # ── Raw payloads (full GitHub API data — for audit and debugging) ─────────
    ...
    milestone: Mapped[Milestone] = relationship(
        "Milestone", back_populates="evidence_records"
    )
